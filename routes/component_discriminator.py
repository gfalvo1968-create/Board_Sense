...
SPIKE Vision v0.7 is deliberately conservative. Named regions need shape plus
supporting visual evidence. Round-feature logic now suppresses capacitor labels on
strong solder-side views and requires body-like appearance, not circles alone.
"""
import cv2
import numpy as np
...
def discriminate_components(image_path):
    result={"ic_like":0,"capacitor_like":0,"contact_pad_like":0,"solder_joint_like":0,"solder_side_likelihood":0,"contact_pattern_score":0.0,"transformer_relay_like":0,"small_component_like":0,"uncertain_like":0,"dominant_family":"unknown","logic_component_ratio":0.0,"power_component_ratio":0.0,"regions":[],"notes":[]}
    try:
        ...
        _,dark=cv2.threshold(blur,82,255,cv2.THRESH_BINARY_INV);dark=cv2.bitwise_and(dark,mask);dark=cv2.morphologyEx(dark,cv2.MORPH_OPEN,np.ones((3,3),np.uint8));contours,_=cv2.findContours(dark,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE);ic_like=block_like=small_like=uncertain_like=0;regions=[]
        for c in contours:
            ...
            if .001<=ar<=.040 and rect>=.70 and aspect<=3.6 and darkness>=.58 and ed>=.05:
            ...
            elif .008<=ar<=.16 and rect>=.50:uncertain_like+=1
        ...
        if circles is not None:
            for cx,cy,r in np.round(circles[0]).astype(int):
                ...
                # A capacitor candidate must look like a component body, not merely
                # a detected circle. Tiny/bright low-saturation rings are rejected.
        ...
        # Strong solder-side context vetoes named capacitor bodies from this view.
        suppressed_caps=0
        ...
        for cx,cy,r,ed,ms,mv in caps:
            ...
            if conf>=64:regions.append({"type":"Capacitor-like component body","x":int((cx-r)*inv),"y":int((cy-r)*inv),"w":int(r*2*inv),"h":int(r*2*inv),"confidence":conf})
        ...
        capacitor_like,contact_pad_like=len(caps),len(contacts);result.update({"ic_like":ic_like,"capacitor_like":capacitor_like,"contact_pad_like":contact_pad_like,"solder_joint_like":solder_count,"solder_side_likelihood":solder_side_likelihood,"contact_pattern_score":round(pattern_score,3),"transformer_relay_like":block_like,"small_component_like":small_like,"uncertain_like":uncertain_like});result["regions"]=sorted(regions,key=lambda i:(i["confidence"],i["w"]*i["h"]),reverse=True)[:24];total=max(ic_like+capacitor_like+block_like,1);result["logic_component_ratio"]=round(ic_like/total,3);result["power_component_ratio"]=round((capacitor_like+block_like)/total,3)
        if ic_like>=3 and ic_like>capacitor_like+block_like:result["dominant_family"]="logic_ic"
        elif capacitor_like+block_like>=4 and capacitor_like+block_like>ic_like*1.35:result["dominant_family"]="power_components"
        elif ic_like or capacitor_like or block_like:result["dominant_family"]="mixed"
        result["notes"].append("SPIKE Vision v0.7 body-before-label filtering is active.")
