...
def _grade_from_reference(score):
    ...
    for rule in rules:
        ...
        if low <= score <= high:
            return {"grade": rule.get("grade", "UNKNOWN"), "recommendation": rule.get("recommended_action", "Manual review recommended."), "pay_dirt_ready": bool(rule.get("pay_dirt_ready", False)), "recovery_signals": rule.get("recovery_signals", []), "grade_notes": rule.get("notes", "")}
...
def _agreement(simple_type, top_hypothesis):
    ...
    sl, wl = simple_type.lower(), weighted_type.lower()
    groups = [("ram", "memory"), ("power", "supply"), ("motherboard", "logic"), ("telecom", "network"), ("expansion", "gold finger"), ("processor",), ("server", "enterprise")]
    agrees = any(any(word in sl for word in g) and any(word in wl for word in g) for g in groups)
    return {"status": "agree" if agrees else "review", "simple_classifier": simple_type, "weighted_hypothesis": weighted_type, "weighted_confidence": top_hypothesis.get("hypothesis_confidence", 0), "message": "Independent classifiers agree on the same broad board family." if agrees else "Independent classifiers disagree; keep the result conservative and review the evidence."}


def _non_board_result(object_gate, photo_quality):
    mode = object_gate.get("mode", "unknown"); label = object_gate.get("label", "Unknown object"); confidence = int(object_gate.get("confidence", 35))
...
def analyze_board(image_path):
    ...
    board_type = classify_board_type(features, visual, motherboard, power); score = calculate_score(features); grade_result = _grade_from_reference(score)
