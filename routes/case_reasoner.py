"""SPIKE Multi-Photo Case Reasoner v0.24."""
...
def _per_view_material_summaries(results):
 ...
 for i,r in enumerate(results or [],1):
  ...
  cond=r.get("condition_and_harvest") or condition_harvest_check(r,mod.get("observations") or {})
  summaries.append({
   "view":i,
   "board_type":r.get("board_type","Unknown Board"),
   "grade":r.get("grade","UNRESOLVED"),
   "score":r.get("score",0),
   "condition":cond.get("condition"),
   "specimen_completeness":cond.get("specimen_completeness"),
   "remaining_value_verdict":cond.get("remaining_value_verdict"),
   "pay_dirt_still_present":bool(cond.get("pay_dirt_still_present",False)),
   "remaining_recovery_targets":cond.get("remaining_recovery_targets",[]),
   "buyer_message":cond.get("buyer_message"),
  })
 return summaries
def _three_answers(result,identity,condition,economics):
 subtype=result.get("equipment_subtype") or {};guard=result.get("recovery_grade_guard") or {};return{"identity":{"question":"What is it?","answer":result.get("board_type","Unknown Board"),"subtype":subtype.get("subtype") or subtype.get("type"),"confidence":result.get("confidence",0),"case_identity_status":identity.get("status")},"recovery":{"question":"What recovery value is physically supported?","grade":result.get("grade","UNRESOLVED"),"score":result.get("score",0),"pay_dirt_ready":bool(result.get("pay_dirt_ready",False)),"condition":condition.get("condition"),"specimen_completeness":condition.get("specimen_completeness"),"remaining_opportunity":condition.get("remaining_recovery_opportunity"),"remaining_value_verdict":condition.get("remaining_value_verdict"),"pay_dirt_still_present":bool(condition.get("pay_dirt_still_present",False)),"buyer_message":condition.get("buyer_message"),"grade_guard":guard.get("model")},"economics":{"question":"What should we do with it?","winner":economics.get("winner"),"needs_values":bool(economics.get("needs_values",False)),"message":economics.get("message")},"separation_rule":"Identity, completeness/condition, remaining recovery value, and economics are independent answers."}
def _mixed_power_control_case(results):
 ...
 supported=bool(len(candidate_views)>=2 and len(power_views)>=2 and max_raw_power>=4 and max_blocks>=1 and(max_rounds>=2 or max_packages>=1))
...
def reconcile_case(results):
 if not results:return{"board_type":"Unknown Board","confidence":0,"model":"SPIKE Multi-Photo Case Reasoner v0.20"}
 identity=verify_same_board(results)
 if identity.get("block_reconciliation"):
  identity_status=identity.get("status")
  clarification=identity_status in {"IDENTITY_CLARIFICATION_NEEDED","IDENTITY_UNCERTAIN"}
  uncertain=identity_status=="IDENTITY_UNCERTAIN"
  answer="Identity Evidence Needed" if uncertain else("Identity Clarification Required" if clarification else "Multiple Boards / Case Split Required")
  recommendation=(identity.get("identity_next_step") if clarification else "Do not combine these photos. Separate them into one case per physical board and analyze again.")
  material=_per_view_material_summaries(results)
  return{"status":"case_identity_clarification" if clarification else "case_identity_failed","board_type":answer,"grade":"UNRESOLVED","confidence":0,"score":0,"recommendation":recommendation,"same_board_verification":identity,"per_view_material_analysis":material,"three_answers":{"identity":{"question":"What is it?","answer":answer,"case_identity_status":identity_status},"recovery":{"question":"What recovery value is physically supported?","grade":"CASE COMBINATION WITHHELD","per_view_material_analysis_available":True,"view_material_summaries":material,"message":"SPIKE will not combine recovery grades across unverified identities, but it still reports what each usable view physically supports."},"economics":{"question":"What should we do with it?","winner":None}},"model":"Board Sense v4.2 + SPIKE Multi-Photo Case Reasoner v0.22 + Same-Board Verification Gate v1.1"}
 hard=[]
 ...
 for key in("processor","large_ic_chips","dense_component_board","possible_motherboard","confirmed_slot_bank","ic_signal_confirmed","possible_power_board","power_board","power_stage_present","mixed_power_control_candidate"):cs[key]=bool(cs.get(key) or any((r.get("signals") or {}).get(key) for r in results))
 cs["component_count"]=max([int((r.get("signals") or {}).get("component_count",0) or 0) for r in results]+[int(cs.get("component_count",0) or 0)])
 cs["component_density"]=max([float((r.get("signals") or {}).get("component_density",0) or 0) for r in results]+[float(cs.get("component_density",0) or 0)])
 logic_reference_views=[];processor_reference_views=[]
 for i,r in enumerate(results,1):
  ri=r.get("reference_intelligence") or {};matches=ri.get("matches") or [];hypotheses=ri.get("hypotheses") or ri.get("ranked_hypotheses") or []
  has_bga=any(str(m.get("category","")).lower().startswith(("bga","processor")) for m in matches if isinstance(m,dict))
  hyp_text=" ".join(str(h) for h in hypotheses).lower()
  top_text=str(ri.get("top_hypothesis") or ri.get("top_match") or "").lower()
  has_logic_hyp=any(x in hyp_text+" "+top_text for x in ("motherboard","logic board","processor-like region","independent logic support"))
  has_processor_hyp="processor-like region" in hyp_text+" "+top_text
  if has_bga or has_logic_hyp:logic_reference_views.append(i)
  if has_processor_hyp:processor_reference_views.append(i)
 reference_logic=bool(logic_reference_views)
 if reference_logic:cs["large_ic_chips"]=True;cs["ic_signal_confirmed"]=True
 if len(set(logic_reference_views))>=2:cs["dense_component_board"]=True
 if len(set(processor_reference_views))>=2:cs["processor"]=True
 ...
 combined["three_answers"]=_three_answers(combined,identity,condition,combined["recovery_economics"]);combined["model"]="Board Sense v4.2 + SPIKE Multi-Photo Case Reasoner v0.24 + Same-Board Verification Gate v1.1 + Physical Fingerprint v0.3 + SPIKE Vision v1.7 + Strict Edge Contact Geometry v0.2 + Reference Reasoner v5 + Board Blueprint v1.0 + Power Topology v0.3 + Motherboard Detector v0.3 + Cross-View Harvest Corroborator v0.1 + Decision Guard v0.3 + Equipment Subtype v0.4 + Recovery Grade Guard v0.5 + Condition & Harvest v0.3 + Recovery Economics v0.2";return combined
