"""SPIKE Multi-Photo Case Reasoner v0.15."""
...
def _evidence_signature(r):
 s=r.get("signals") or {};mb=r.get("motherboard") or {};return(str(r.get("board_type","Unknown Board")),bool(s.get("ram") or s.get("possible_ram")),bool(s.get("processor")),bool(s.get("large_ic_chips")),bool(s.get("possible_power_board") or s.get("power_board") or s.get("power_stage_present")),bool(s.get("gold_fingers") or s.get("gold_finger_edge")),bool(s.get("possible_motherboard")),bool(s.get("confirmed_slot_bank") or mb.get("confirmed_slot_bank")),bool(mb.get("edge_connector_bank")))
def _view_role(r):
...
def _mixed_power_control_case(results):
 power_views=[];logic_views=[];edge_views=[];candidate_views=[];max_power=0;max_raw_power=0;max_blocks=0;max_rounds=0
 for i,r in enumerate(results,1):
  s=r.get("signals") or {};p=r.get("power") or {};label=str(r.get("board_type","")).lower()
  ps=max(int(s.get("power_score",0) or 0),int(p.get("power_score",0) or 0));raw_ps=max(int(s.get("raw_power_score",0) or 0),int(p.get("raw_power_score",0) or 0),ps);blocks=max(int(s.get("large_component_regions",0) or 0),int(p.get("large_component_regions",0) or 0));rounds=max(int(s.get("large_round_components",0) or 0),int(p.get("large_round_components",0) or 0));stage=bool(s.get("power_stage_present") or p.get("power_stage_present"));mixed_candidate=bool(s.get("mixed_power_control_candidate") or p.get("mixed_power_control_candidate"));max_power=max(max_power,ps);max_raw_power=max(max_raw_power,raw_ps);max_blocks=max(max_blocks,blocks);max_rounds=max(max_rounds,rounds)
  if stage or mixed_candidate or (blocks>=1 and(raw_ps>=3 or rounds>=2 or blocks>=2)) or "power-control" in label:power_views.append(i)
...
def reconcile_case(results):
 ...
 for i,r in enumerate(results,1):
  ...
  sig=_evidence_signature(r);repeat=signatures.get(sig,0);signatures[sig]=repeat+1;mb=r.get("motherboard") or {};s=r.get("signals") or {};view_summaries.append({"view":i,"role":_view_role(r),"board_type":r.get("board_type"),"confidence":r.get("confidence"),"grade":r.get("grade"),"power_stage_present":bool(s.get("power_stage_present")),"mixed_power_control_candidate":bool(s.get("mixed_power_control_candidate")),"raw_power_score":int(s.get("raw_power_score",0) or 0),"confirmed_slot_bank":bool(s.get("confirmed_slot_bank") or mb.get("confirmed_slot_bank")),"edge_connector_bank":bool(mb.get("edge_connector_bank")),"modification_status":mod.get("status","not_evaluated"),"evidence_independence":"primary" if repeat==0 else "corroborating_repeat"})
 cross=_cross_view_harvest(results,observations);cs=combined.setdefault("signals",{});cp=combined.setdefault("power",{})
 for key in("processor","large_ic_chips","dense_component_board","gold_fingers","gold_finger_edge","possible_power_board","power_board","power_stage_present","mixed_power_control_candidate"):cs[key]=bool(cs.get(key) or any((r.get("signals") or {}).get(key) for r in results))
 cs["power_score"]=max([int((r.get("signals") or {}).get("power_score",0) or 0) for r in results]+[int(cs.get("power_score",0) or 0),mixed["max_power_score"]]);cs["raw_power_score"]=max([int((r.get("signals") or {}).get("raw_power_score",0) or 0) for r in results]+[int(cs.get("raw_power_score",0) or 0),mixed["max_raw_power_score"]])
 ...
 combined["three_answers"]=_three_answers(combined,identity,condition,combined["recovery_economics"]);combined["model"]="Board Sense v3.9 + SPIKE Multi-Photo Case Reasoner v0.15 + Same-Board Verification Gate v0.6 + Physical Fingerprint v0.3 + Power Topology v0.2 + Cross-View Harvest Corroborator v0.1 + Decision Guard v0.3 + Equipment Subtype v0.3 + Recovery Grade Guard v0.3 + Condition & Harvest v0.2 + Recovery Economics v0.2";return combined
