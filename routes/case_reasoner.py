"""SPIKE Multi-Photo Case Reasoner v0.22."""
...
def reconcile_case(results):
 ...
 reference_logic=any(any(str(m.get("category","")).lower().startswith(("bga","processor")) for m in ((r.get("reference_intelligence") or {}).get("matches") or []) if isinstance(m,dict)) for r in results)
 ...
 combined["three_answers"]=_three_answers(combined,identity,condition,combined["recovery_economics"]);combined["model"]="Board Sense v4.2 + SPIKE Multi-Photo Case Reasoner v0.21 + Same-Board Verification Gate v0.6 + Physical Fingerprint v0.3 + SPIKE Vision v1.0 + Strict Edge Contact Geometry v0.2 + Reference Reasoner v5 + Board Blueprint v0.7 + Power Topology v0.3 + Motherboard Detector v0.3 + Cross-View Harvest Corroborator v0.1 + Decision Guard v0.3 + Equipment Subtype v0.3 + Recovery Grade Guard v0.3 + Condition & Harvest v0.3 + Recovery Economics v0.2";return combined
