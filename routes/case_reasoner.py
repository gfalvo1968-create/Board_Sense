"""SPIKE Multi-Photo Case Reasoner v0.14."""
...
def _mixed_power_control_case(results):
 power_views=[];logic_views=[];edge_views=[];max_power=0;max_blocks=0;max_rounds=0
 for i,r in enumerate(results,1):
  s=r.get("signals") or {};p=r.get("power") or {};label=str(r.get("board_type","")).lower()
  ps=max(int(s.get("power_score",0) or 0),int(p.get("power_score",0) or 0));blocks=max(int(s.get("large_component_regions",0) or 0),int(p.get("large_component_regions",0) or 0));rounds=max(int(s.get("large_round_components",0) or 0),int(p.get("large_round_components",0) or 0));max_power=max(max_power,ps);max_blocks=max(max_blocks,blocks);max_rounds=max(max_rounds,rounds)
...
def reconcile_case(results):
 ...
 if mixed["supported"] and not hard:
  combined["board_type"]="Power-Control / Controller Board";combined["board_type_reason"]="Across verified same-board views, substantial power-stage hardware and control/logic evidence are both present. This structural topology outranks isolated edge-connector or gold-colour hints.";cs["mixed_power_control_topology"]=True;cs["possible_power_board"]=True;cs["power_board"]=True
  # Preserve the reconciled topology in the canonical power object as well as
  # signals so downstream subtype/Jury logic sees the same case-level evidence.
  cp["possible_power_board"]=True;cp["power_score"]=max(int(cp.get("power_score",0) or 0),mixed["max_power_score"]);cp["large_component_regions"]=max(int(cp.get("large_component_regions",0) or 0),mixed["max_power_blocks"]);cp["large_round_components"]=max(int(cp.get("large_round_components",0) or 0),mixed["max_large_round_components"]);cp["case_reconciled_power_topology"]=True
 ...
 combined["three_answers"]=_three_answers(combined,identity,condition,combined["recovery_economics"]);combined["model"]="Board Sense v3.7 + SPIKE Multi-Photo Case Reasoner v0.14 + Same-Board Verification Gate v0.6 + Physical Fingerprint v0.3 + Cross-View Harvest Corroborator v0.1 + Decision Guard v0.3 + Equipment Subtype v0.3 + Recovery Grade Guard v0.3 + Condition & Harvest v0.2 + Recovery Economics v0.2";return combined
