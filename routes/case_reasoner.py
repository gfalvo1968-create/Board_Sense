"""SPIKE Multi-Photo Case Reasoner v0.16."""
...
def _mixed_power_control_case(results):
 power_views=[];logic_views=[];edge_views=[];candidate_views=[];package_views=[];max_power=0;max_raw_power=0;max_blocks=0;max_rounds=0;max_packages=0
 for i,r in enumerate(results,1):
 ...
 return {"supported":supported,"power_views":power_views,"logic_views":logic_views,"mixed_candidate_views":candidate_views,"large_power_package_views":package_views,"edge_signal_views":edge_views,"max_power_score":max_power,"max_raw_power_score":max_raw_power,"max_power_blocks":max_blocks,"max_large_round_components":max_rounds,"max_large_power_package_like":max_packages,"rule":"After same-board verification, raw physical power-stage evidence survives logic penalties. A large power package is corroborating evidence only; power-stage plus control/logic evidence outranks an isolated edge/gold-colour identity hint."}
def reconcile_case(results):
 if not results:return{"board_type":"Unknown Board","confidence":0,"model":"SPIKE Multi-Photo Case Reasoner v0.16"}
 identity=verify_same_board(results)
 if identity.get("block_reconciliation"):return{"status":"case_identity_failed","board_type":"Multiple Boards / Case Split Required","grade":"UNRESOLVED","confidence":0,"score":0,"recommendation":"Do not combine these photos. Separate them into one case per physical board and analyze again.","same_board_verification":identity,"three_answers":{"identity":{"question":"What is it?","answer":"Multiple Boards / Case Split Required","case_identity_status":identity.get("status")},"recovery":{"question":"What recovery value is physically supported?","grade":"WITHHELD"},"economics":{"question":"What should we do with it?","winner":None}},"model":"Board Sense v4.0 + SPIKE Multi-Photo Case Reasoner v0.16 + Same-Board Verification Gate v0.6"}
 hard=[]
 ...
 combined["three_answers"]=_three_answers(combined,identity,condition,combined["recovery_economics"]);combined["model"]="Board Sense v4.0 + SPIKE Multi-Photo Case Reasoner v0.16 + Same-Board Verification Gate v0.6 + Physical Fingerprint v0.3 + SPIKE Vision v0.8 + Power Topology v0.3 + Motherboard Detector v0.3 + Cross-View Harvest Corroborator v0.1 + Decision Guard v0.3 + Equipment Subtype v0.3 + Recovery Grade Guard v0.3 + Condition & Harvest v0.2 + Recovery Economics v0.2";return combined
