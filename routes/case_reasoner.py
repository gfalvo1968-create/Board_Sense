"""SPIKE Multi-Photo Case Reasoner v0.3.

Combines several photographs of one physical board. Strong structural evidence
outranks repeated weak hints, repeated views have diminishing authority, and the
final recovery grade is checked against physically confirmed logic evidence.
"""
...
def reconcile_case(results):
    if not results:return {"board_type":"Unknown Board","confidence":0,"model":"SPIKE Multi-Photo Case Reasoner v0.3"}
    hard=[]
    ...
    for i,r in enumerate(results,1):
        ...
        for name,obs in (mod.get("observations") or {}).items():
            ...
            if old is None or str(obs.get("status","")).lower() in ("removed","cut","harvested","missing_confirmed"):observations[name]=obs
    ...
    combined["condition_and_harvest"]=condition_harvest_check(combined,observations);combined["equipment_subtype"]=infer_equipment_subtype(combined)
    # Case-level corroboration: a useful feature seen clearly in any independent
    # view may support recovery economics even if the representative photo misses it.
    cs=combined.setdefault("signals",{})
    for key in ("processor","large_ic_chips","dense_component_board","gold_fingers"):
        cs[key]=bool(cs.get(key) or any((r.get("signals") or {}).get(key) for r in results))
    combined=apply_recovery_grade_guard(combined)
    unique=len(signatures);combined["case_analysis"]={"mode":"same_board_multi_photo","views_analyzed":len(results),"independent_evidence_patterns":unique,"view_summaries":view_summaries,"duplicate_evidence_guard":{"active":True,"rule":"Repeated equivalent views corroborate but do not multiply authority at full weight."},"message":"Multiple photographs were treated as one physical board case. Structural evidence outranks repeated weak hints; view diversity outranks photo count."}
    best=max(float(r.get("confidence",0) or 0) for r in results);combined["confidence"]=min(98,max(float(combined.get("confidence",0) or 0),best));combined["model"]="Board Sense v2.6 + SPIKE Multi-Photo Case Reasoner v0.3 + Recovery Grade Guard v0.1 + Equipment Subtype v0.1 + Verification v0.1"
    return combined
