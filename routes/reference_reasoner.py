"""Board Sense reference knowledge reasoner, SPIKE weighted evidence v4."""
...
def build_reference_matches(features, visual, motherboard, power, board_type):
    ...
    power_like=bool(features.get("power_board") or features.get("possible_power_board") or power.get("possible_power_board"));power_score=max(int(features.get("power_score",0) or 0),int(power.get("power_score",0) or 0));large_round=int(power.get("large_round_components",0) or 0);large_regions=int(power.get("large_component_regions",0) or 0);component_count=int(features.get("component_count",0) or 0);aspect=float(visual.get("aspect_ratio",0) or 0);logic=bool(processor or large_ics or dense)
    mixed_power_control=bool(logic and (power_like or power_score>=4) and structure<6)
    ...
    if mixed_power_control:
        ...
        _add(h,"Expansion / Gold Finger Card",-2,"Mixed power-control topology outweighs an unconfirmed edge-colour cue")
    ...
    if gold:
        for item in knowledge.get("gold_fingers",[]):
            if item.get("category") in ("High Quality Gold Finger Card","Full Gold Fingers"):matches.append(_compact_match(item,"Confirmed repeated rectangular edge contacts"));break
    ...
    return {"matches":matches,"match_count":len(matches),"hypotheses":ranked[:5],"top_hypothesis":top,"hypothesis_margin":margin,"telecom_pattern":False,"board_type_context":board_type.get("type","General PCB"),"gold_finger_geometry_confirmed":bool(gold),"motherboard_structure_score":structure,"mixed_power_control_supported":mixed_power_control,"reasoning_version":"SPIKE-weighted-evidence-v4"}
