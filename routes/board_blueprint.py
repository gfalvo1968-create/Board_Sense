"""Board Blueprint v0.8: conservative context, component, and connector guards."""
...
def generate_blueprint(image_path,component_regions,output_dir):
 ...
 for n,r in enumerate(selected,1):
  ...
  index.append(item)
 cv2.imwrite(str(output_path),image);return{"available":True,"image_filename":output_name,"component_index":index,"marker_count":len(index),"candidate_region_count":len(all_regions),"mode":"Board Blueprint v0.8 + Context Guard v0.1 + Component Guard v0.5","context_guard":{"active":True,"rule":"Generic geometry stays generic until equipment-specific evidence independently earns a subtype label.","motherboard_structure_score":motherboard.get("motherboard_structure_score",0)},"component_guard":{"active":True,"rule":"Board-edge connector labels require explicit repeated, aligned connector-body proof across a meaningful PCB-perimeter run."},"note":"Blueprint prioritizes board-defining and likely value-bearing structures. Connector/contact labels describe visible geometry only; metal content is not visually assayed."}
