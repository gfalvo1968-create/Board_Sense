...
def generate_blueprint(image_path,component_regions,output_dir):
 ...
 frame_gate=inspect_frame(str(image_path))
 if frame_gate.get("block_analysis"):
  return{"available":False,"message":"Blueprint withheld: more than one physical board or overlapping PCB regions may be present in this photo. Retake with exactly one board in frame.","component_index":[],"frame_identity_gate":frame_gate,"mode":"Board Blueprint v1.0 + Single-Frame Identity Gate v0.1"}
