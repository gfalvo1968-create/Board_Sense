"""Board Blueprint v1.0: density-aware component guards plus single-frame identity protection."""
...
def generate_blueprint(image_path,component_regions,output_dir,resolved_bottleneck=False):
 ...
 metrics=frame_gate.get("metrics") or {}
 clarification_safe=bool(resolved_bottleneck and metrics.get("bottleneck_split_trigger") and not any(metrics.get(key) for key in ("base_two_region_trigger","multiscale_split_trigger","secondary_plane_trigger")))
 if frame_gate.get("block_analysis") and not clarification_safe:
  return{"available":False,"message":"Blueprint withheld: more than one physical board or overlapping PCB regions may be present in this photo. Retake with exactly one board in frame.","component_index":[],"frame_identity_gate":frame_gate,"mode":"Board Blueprint v1.0 + "+frame_version}
