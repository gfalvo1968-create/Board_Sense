...
def _component_geometry_guard(r,image_w=None,image_h=None):
 ...
 if t in("Rear I/O / connector bank","Board-edge connector bank","Motherboard slot / socket bank") and image_w and image_h:
  x=max(0,int(r.get("x",0)));y=max(0,int(r.get("y",0)));area_ratio=(w*h)/max(1,image_w*image_h);margin=max(10,int(min(image_w,image_h)*.065));side=r.get("edge_side")
  ...
  # Structural edge-bank regions are accepted only when the source detector says
  # which side they belong to AND the bounding box remains a narrow perimeter strip.
  side_consistent=bool(side in distances and distances[side]<=margin)
  narrow_strip=(w<=image_w*.30) if side in("left","right") else((h<=image_h*.30) if side in("top","bottom") else False)
  if t in("Rear I/O / connector bank","Board-edge connector bank") and (not near_edge or not side_consistent or not narrow_strip or area_ratio>.12):
...
def generate_blueprint(image_path,component_regions,output_dir):
 ...
 for n,r in enumerate(selected,1):
  ...
  index.append(item)
 cv2.imwrite(str(output_path),image);return{"available":True,"image_filename":output_name,"component_index":index,"marker_count":len(index),"candidate_region_count":len(all_regions),"mode":"Board Blueprint v0.7 + Context Guard v0.1 + Component Guard v0.4","context_guard":{"active":True,"rule":"Generic geometry stays generic until equipment-specific evidence independently earns a subtype label.","motherboard_structure_score":motherboard.get("motherboard_structure_score",0)},"component_guard":{"active":True,"rule":"Connector labels require side-specific narrow PCB-perimeter geometry; interior solder-side patterns cannot become connector banks."},"note":"Blueprint prioritizes board-defining and likely value-bearing structures. Connector/contact labels describe visible geometry only; metal content is not visually assayed."}
