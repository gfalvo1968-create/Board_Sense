"""SPIKE Same-Board Verification Gate v0.4.
Classification contradictions and physical-board fingerprints are evaluated before
multi-photo evidence is allowed to merge. The gate now exposes whole-view coverage,
a conflict graph, and the exact next photo SPIKE needs when identity is uncertain.
"""
...
def verify_same_board(results):
 ...
 conflict_graph=[{"view":i,"conflict_degree":per_view.get(i,0)} for i in range(1,n+1) if per_view.get(i,0)>0];cluster_conflict=any(v>=2 for v in per_view.values())
 ...
 if uncertain_physical or len(unique)>1:
  ...
  status="PROBABLY_SAME_BOARD";conf=82;reasons.append("No strong cross-view family or physical-geometry contradiction was found.");next_step="No extra identity photo required unless the case result remains otherwise uncertain."
 return {"version":"SPIKE Same-Board Verification Gate v0.4","status":status,"same_board":True if status=="PROBABLY_SAME_BOARD" else None,"confidence":conf,"block_reconciliation":False,"families":families,"whole_view_count":len(whole_views),"whole_view_indices":whole_views,"physical_pair_checks":physical,"conflict_graph":conflict_graph,"conflicting_views":sorted(conflict_views),"identity_next_step":next_step,"reasons":reasons,"rule":"Absence of contradiction is not proof of identity; uncertain cases remain flagged."}
