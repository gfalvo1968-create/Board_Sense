"""SPIKE Same-Board Verification Gate v0.5.
Classification contradictions and usable physical fingerprints are checked before
multi-photo evidence is merged. Hard geometry blocks require a coherent outlier,
not merely two unrelated pair conflicts.
"""
...
def verify_same_board(results):
 ...
 # Geometry alone hard-blocks only when one usable whole-board view disagrees with
 # at least two others. Two disconnected conflicts may be front/back or perspective
 # effects and therefore remain uncertain rather than becoming a false Frankenstein.
 if outliers:
  strong_conflict=True;reasons.append("A whole-board view conflicts with at least two other usable board geometries, forming a coherent physical outlier.")
 elif conflicts:
  uncertain_physical=True;reasons.append("Physical fingerprint disagreement exists, but it does not yet form a corroborated multiple-board cluster.")
 if strong_conflict:
  return {"version":"SPIKE Same-Board Verification Gate v0.5","status":"MULTIPLE_BOARDS_SUSPECTED","same_board":False,"confidence":92,"block_reconciliation":True,"families":families,"whole_view_count":len(whole_views),"whole_view_indices":whole_views,"physical_pair_checks":physical,"conflict_graph":conflict_graph,"conflicting_views":sorted(conflict_views),"identity_next_step":"Split the photos by physical board and start a separate case for each board.","reasons":reasons,"rule":"Semantic contradiction or a corroborated physical outlier is required before reconciliation is hard-blocked."}
